// Sofa Scrap — GitHub/Render ROOT-ONLY build — V60.6 CACHE-FIRST FILTER ENGINE
...
if(action==='inspect_import' && req.method==='POST'){
    ...
    if(filtroId>0){
      ...
      const runCacheFilter=async (games, filterObj)=>{
        ...
        // V60.6: consultar o cache por métrica, e não fazer uma requisição
        ...
        for(const metricId of metricIds){
          ...
          let rows=[];
          try{
            rows=await getAll(`metricas_cache?select=*&metrica_id=eq.${metricId}`,5000,50000);
          }catch(e){
