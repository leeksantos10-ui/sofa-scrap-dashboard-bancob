...
function fairPnl(j,market,stake=1,commission=.045){const f=fairOdd(j,market);const mc=marketMap[market];if(f===null||!mc)return null;const g=num(j.GOLS_CASA_FT),h=num(j.GOLS_VISITANTE_FT);if(g===null||h===null)return null;const green=mc.win(g,h);const gross=(green?f-1:-1)*stake;const net=gross>0?gross*(1-commission):gross;return {fairOdd:f,fairPnlGross:gross,fairPnlNet:net};}
  try {
    ...
    if(action==='metrics'){
      // Aba Métricas: a fonte oficial da lista é metricas_config.
      // Não misturar a tabela legada "metricas", que fazia a tela mostrar
      // apenas registros antigos e escondia as configurações criadas no Banco B.
      const cfg=await getAll('metricas_config?select=id,nome,descricao,campo,contexto,janela_jogos,minimo_jogos,operacao,ativa,formula,criado_em,atualizado_em,equipe&order=id.asc',1000,5000);
      const fr=await Promise.allSettled([
        getAll('metricas_campos?select=*&order=id.asc',1000,5000)
      ]);
      const dbFields=fr[0].status==='fulfilled'?fr[0].value:[];
      const normalizedCfg=cfg.map(x=>({
        ...x,
        id:x.id,
        nome:x.nome||`Configuração ${x.id}`,
        campo:x.campo||'',
        contexto:x.contexto||'GERAL',
        janela:x.janela_jogos??5,
        minimo:x.minimo_jogos??1,
        operacao:x.operacao||'MEDIA',
        ativa:x.ativa??true,
        fonte:'metricas_config'
      }));
      ...
      return res.status(200).json({
        metricas:normalizedCfg,
        configs:cfg,
        campose:dbFields.length?dbFields:fallbackFields,
        fieldsSource:dbFields.length?'Banco B':'fallback oficial',
        warnings:{metricas:false,configs:false,campos:fr[0].status==='rejected'}
      });
    }
