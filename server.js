// Sofa Scrap — GitHub/Render ROOT-ONLY build — V60.9 DIRECT-CACHE FILTER ENGINE
...
if(action==='inspect_import' && req.method==='POST'){
    ...
    if(filtroId>0){
      ...
      // V60.5: motor de filtro cache-first. Primeiro tenta avaliar as condições
      // usando metricas_cache em lote. Só para jogos/condições sem cache usa
      // o motor V59 por métrica, evitando executar_filtro_jogos e avaliar_filtro
      // como caminho principal. Se a estrutura do filtro for incompatível,
      // preservamos o fallback oficial para não alterar o resultado histórico.
      const normalizeText=v=>String(v??'').trim().toUpperCase();
      const toNum=v=>{const n=Number(v);return Number.isFinite(n)?n:null};
      const compareValue=(a,op,b,pct=null)=>{
        const x=toNum(a), y=toNum(b); if(x===null||y===null)return false;
        const o=normalizeText(op||'>').replace('=>','>=').replace('=<','<=');
        if(o==='>')return x>y; if(o==='>=')return x>=y; if(o==='<')return x<y; if(o==='<=')return x<=y;
        if(o==='='||o==='=='||o==='===')return x===y; if(o==='!='||o==='<>')return x!==y;
        if(o==='BETWEEN')return x>=y && x<=toNum(pct);
        return false;
      };
      const cacheValue=row=>{
        for(const k of ['valor','value','resultado','valor_metrica','valor_calculado','media','media_valor']){
          if(row&&row[k]!==undefined&&row[k]!==null){const n=toNum(row[k]);if(n!==null)return n;}
        }
        return null;
      };
      const cacheMatches=(row,cfg,cond)=>{
        if(!row)return false;
        if(Number(row.metrica_id)!==Number(cond.metrica_id))return false;
        if(Number(row.jogo_id)!==Number(cfg.jogo_id))return false;
        const eq=normalizeText(cond.equipe||'CASA'), ctx=normalizeText(cond.contexto||'GERAL');
        const rEq=normalizeText(row.equipe||eq), rCtx=normalizeText(row.contexto||ctx);
        if(rEq!==eq||rCtx!==ctx)return false;
        const win=Number(cond.janela||5), min=Number(cond.minimo||3), op=normalizeText(cond.operacao||'MEDIA');
        if(row.janela!==undefined&&Number(row.janela)!==win)return false;
        if(row.minimo!==undefined&&Number(row.minimo)!==min)return false;
        if(row.operacao!==undefined&&normalizeText(row.operacao)!==op)return false;
        return cacheValue(row)!==null;
      };
      const runCacheFilter=async (games, filterObj)=>{
        const conds=(filterObj?.condicoes||[]).filter(Boolean);
        const filtro=filterObj?.filtro||{};
        const nos=(filterObj?.nos||[]).filter(n=>n.tipo==='CONDICAO');
        const opGrupo=normalizeText(filtro.operador_logico||filtro.operador||'E');
        // O motor em lote abaixo é intencionalmente conservador: apenas filtros
        // lineares de condições são avaliados aqui. Estruturas aninhadas/OU
        // complexas continuam no fallback oficial.
        if(!conds.length||nos.length!==conds.length)return null;
        if(conds.some(c=>!c.metrica_id))return null;
        if(!['E','OU'].includes(opGrupo))return null;
        const metricIds=[...new Set(conds.map(c=>Number(c.metrica_id)).filter(Number.isInteger))];
        const metricCfgRows=await get(`metricas_config?id=in.(${metricIds.join(',')})&select=*`);
        const metricCfg=new Map((metricCfgRows||[]).map(x=>[Number(x.id),x]));
        if(metricIds.some(id=>!metricCfg.has(id)))return null;
        const gamesById=new Map(games.map(j=>[String(j.id),j]));
        const ids=new Set([...gamesById.keys()]);

        // V60.9: caminho rápido para filtros lineares com comparação direta.
        // Em vez de baixar todo o cache, o Supabase aplica o operador sobre
        // `valor` e devolve somente os jogo_id que passam na condição.
        // Isso evita trazer 15k+ linhas para o Render quando o cache já está
        // completo. Estruturas com métrica comparada continuam no motor
        // cache-first abaixo.
        const directConditionQuery=async (c)=>{
          if(c.metrica_comparada_id) return null;
          const metricId=Number(c.metrica_id);
          if(!Number.isInteger(metricId)) return null;
          const mc=metricCfg.get(metricId)||{};
          const eq=c.equipe||mc.equipe||'CASA';
          const ctx=c.contexto||mc.contexto||'GERAL';
          const win=Number(c.janela||mc.janela_jogos||5);
          const min=Number(c.minimo||mc.minimo_jogos||3);
          const op=c.operacao||mc.operacao||'MEDIA';
          const operator=normalizeText(c.operador||'>').replace('=>','>=').replace('=<','<=');
          const target=toNum(c.valor);
          if(target===null) return null;
          const opMap={'>':'gt','>=':'gte','<':'lt','<=':'lte','=':'eq','==':'eq','===':'eq','!=':'neq','<>':'neq'};
          const dbOp=opMap[operator];
          if(!dbOp && operator!=='BETWEEN') return null;
          let q=`metricas_cache?select=jogo_id&metrica_id=eq.${metricId}&equipe=eq.${encodeURIComponent(eq)}&contexto=eq.${encodeURIComponent(ctx)}&janela=eq.${win}&minimo=eq.${min}&operacao=eq.${encodeURIComponent(op)}`;
          if(operator==='BETWEEN'){
            const upper=toNum(c.percentual);
            if(upper===null) return null;
            q+=`&valor=gte.${encodeURIComponent(String(target))}&valor=lte.${encodeURIComponent(String(upper))}`;
          }else{
            q+=`&valor=${dbOp}.${encodeURIComponent(String(target))}`;
          }
          const rows=await getAll(q,1000,50000);
          return new Set((rows||[]).map(r=>String(r.jogo_id)).filter(id=>ids.has(id)));
        };

        const directPossible=conds.every(c=>!c.metrica_comparada_id && c.metrica_id && c.valor!==undefined && c.valor!==null);
        if(directPossible){
          const sets=await Promise.all(conds.map(directConditionQuery));
          if(sets.every(Boolean)){
            const approvedIdsDirect=new Set();
            if(opGrupo==='E'){
              for(const id of ids){if(sets.every(st=>st.has(id)))approvedIdsDirect.add(id);}
            }else{
              for(const st of sets)for(const id of st)approvedIdsDirect.add(id);
            }
            const approved=games.filter(j=>approvedIdsDirect.has(String(j.id)));
            return {approved,cacheMissing:0,metricIds,cacheDirect:true};
          }
        }

        const valuesByMetric=new Map();
        // V60.7: consultar o cache por métrica em páginas compatíveis com o limite do PostgREST, e não fazer uma requisição
        // para cada bloco de IDs de jogos. O padrão anterior podia gerar
        // centenas de requests sequenciais para um único filtro e acabava
        // em "fetch failed"/timeout no Render.
        const cacheKey=(jId,eq,ctx,win,min,op)=>`${jId}|${normalizeText(eq)}|${normalizeText(ctx)}|${Number(win)}|${Number(min)}|${normalizeText(op)}`;
        const missing=[];
        for(const metricId of metricIds){
          const map=new Map();
          const related=conds.filter(c=>Number(c.metrica_id)===metricId);
          const configs=[...new Map(related.map(c=>{
            const mc=metricCfg.get(metricId)||{};
            const cfg={equipe:c.equipe||mc.equipe||'CASA',contexto:c.contexto||mc.contexto||'GERAL',janela:Number(c.janela||mc.janela_jogos||5),minimo:Number(c.minimo||mc.minimo_jogos||3),operacao:c.operacao||mc.operacao||'MEDIA'};
            return [cacheKey('',cfg.equipe,cfg.contexto,cfg.janela,cfg.minimo,cfg.operacao),cfg];
          }))].map(x=>x[1]);
          try{
            for(const cfg of configs){
              const q=`metricas_cache?select=jogo_id,equipe,contexto,janela,minimo,operacao,valor&metrica_id=eq.${metricId}&equipe=eq.${encodeURIComponent(cfg.equipe)}&contexto=eq.${encodeURIComponent(cfg.contexto)}&janela=eq.${cfg.janela}&minimo=eq.${cfg.minimo}&operacao=eq.${encodeURIComponent(cfg.operacao)}`;
              const rows=await getAll(q,1000,50000);
              for(const row of rows||[]){
                if(!ids.has(String(row.jogo_id)))continue;
                const val=cacheValue(row); if(val===null)continue;
                map.set(cacheKey(row.jogo_id,cfg.equipe,cfg.contexto,cfg.janela,cfg.minimo,cfg.operacao),val);
              }
            }
          }catch(e){
            console.warn(`Falha ao ler metricas_cache para a métrica ${metricId}: ${e.message||e}`);
            return null;
          }
          valuesByMetric.set(metricId,map);
        }
        const getCached=(j,cond)=>{
          const map=valuesByMetric.get(Number(cond.metrica_id)); if(!map)return null;
          const mc=metricCfg.get(Number(cond.metrica_id))||{};
          const eq=cond.equipe||mc.equipe||'CASA';
          const ctx=cond.contexto||mc.contexto||'GERAL';
          const win=Number(cond.janela||mc.janela_jogos||5);
          const min=Number(cond.minimo||mc.minimo_jogos||3);
          const op=cond.operacao||mc.operacao||'MEDIA';
          const key=cacheKey(j.id,eq,ctx,win,min,op);
          return map.has(key)?map.get(key):null;
        };
        for(const j of games){for(const c of conds){if(getCached(j,c)===null)missing.push({j,c});}}
        if(missing.length){
          const concurrency=24;
          for(let i=0;i<missing.length;i+=concurrency){
            const batch=missing.slice(i,i+concurrency);
            await Promise.all(batch.map(async ({j,c})=>{
              const mc=metricCfg.get(Number(c.metrica_id))||{};
              const eq=c.equipe||mc.equipe||'CASA', ctx=c.contexto||mc.contexto||'GERAL';
              let result=null;
              const tipo=normalizeText(mc?.formula?.tipo||'NORMAL');
              if(tipo==='COMPOSTA'){
                const r=await post('rpc/calcular_metrica_composta_filtro_v59',{p_metrica_id:Number(c.metrica_id),p_jogo_id:Number(j.id),p_nivel:1,p_equipe:eq,p_contexto:ctx},'POST');
                result=Array.isArray(r)?r[0]:r;
              }else{
                const r=await post('rpc/calcular_metrica_jogo_filtro_v59',{p_config_id:Number(c.metrica_id),p_jogo_id:Number(j.id),p_equipe:eq,p_contexto:ctx},'POST');
                result=Array.isArray(r)?r[0]:r;
              }
              const val=cacheValue({valor:result});
              if(val!==null)valuesByMetric.get(Number(c.metrica_id))?.set(cacheKey(j.id,eq,ctx,Number(c.janela||mc.janela_jogos||5),Number(c.minimo||mc.minimo_jogos||3),c.operacao||mc.operacao||'MEDIA'),val);
            }));
          }
        }
        const evaluateCondition=(j,c)=>{
          const v=getCached(j,c); if(v===null)return false;
          let target=c.valor;
          if(c.metrica_comparada_id){
            const other=conds.find(x=>Number(x.metrica_id)===Number(c.metrica_comparada_id));
            if(!other)return false;
            const ov=getCached(j,other); if(ov===null)return false;
            target=ov;
          }
          return compareValue(v,c.operador||'>',target,c.percentual);
        };
        const approved=games.filter(j=>{
          const results=conds.map(c=>evaluateCondition(j,c));
          return opGrupo==='OU'?results.some(Boolean):results.every(Boolean);
        });
        return {approved,cacheMissing:missing.length,metricIds};
      };
      try{
        const engine=await runCacheFilter(data,{...await buildFilter(filtroId),filtro:selectedFilter});
        if(engine){
          approvedIds=new Set(engine.approved.map(j=>String(j.id)));
        }
      }catch(e){
        console.warn(`Motor cache-first do filtro #${filtroId} não pôde ser concluído; usando avaliação oficial: ${e.message||e}`);
      }
      if(!approvedIds){
        try{
          const result=await post('rpc/executar_filtro_jogos',{p_filtro_id:filtroId},'POST');
          const normalizeFilterIds=(value)=>{
            let v=value; if(typeof v==='string'){try{v=JSON.parse(v)}catch{}}
            if(Array.isArray(v))return v; if(v==null)return [];
            if(v.jogo_id!=null||v.id!=null)return [v];
            for(const k of ['ids','jogo_ids','results','result','data','rows','jogos']){if(v?.[k]!=null){const x=normalizeFilterIds(v[k]);if(x.length)return x;}}
            return [];
          };
          const ids=normalizeFilterIds(result).map(x=>typeof x==='object'?String(x?.jogo_id??x?.id??''):String(x)).filter(Boolean);
