...
function fairPnl(j,market,stake=1,commission=.045){const f=fairOdd(j,market);const mc=marketMap[market];if(f===null||!mc)return null;const g=num(j.GOLS_CASA_FT),h=num(j.GOLS_VISITANTE_FT);if(g===null||h===null)return null;const green=mc.win(g,h);const gross=(green?f-1:-1)*stake;const net=gross>0?gross*(1-commission):gross;return {fairOdd:f,fairPnlGross:gross,fairPnlNet:net};}
  try {
    ...
    if(action==='engine_test') {
      ...
      const jogos=await getAll(`jogos?select=${select}&order="DATA".asc,"id".asc`,1000,50000);
      ...
      return res.status(200).json({ok:true,engine:'NODE_V60_10_SHADOW',filtro_id:filtroId,filtro_nome:filtro.nome,jogos_total:jogos.length,jogos_avaliados:jogosAvaliados.length,jogos_aprovados:approved.length,ids:approved.slice(0,20),metricas_usadas:[...metricIds],campos_carregados:[...fields],index_chaves:historyIndex.size,index_build_ms:indexBuildMs,tempo_ms:Date.now()-t0,probe,observacao:'Motor novo em modo sombra com índice histórico em memória. Nenhuma função SQL nem resultado do dashboard foi alterado.'});
    }
    if(action==='db_count') {
