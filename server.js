...
function fairPnl(j,market,stake=1,commission=.045){const f=fairOdd(j,market);const mc=marketMap[market];if(f===null||!mc)return null;const g=num(j.GOLS_CASA_FT),h=num(j.GOLS_VISITANTE_FT);if(g===null||h===null)return null;const green=mc.win(g,h);const gross=(green?f-1:-1)*stake;const net=gross>0?gross*(1-commission):gross;return {fairOdd:f,fairPnlGross:gross,fairPnlNet:net};}
  try {
    ...
    const seasonLabel=v=>({ '40557':'2022','48982':'2023','58766':'2024','72034':'2025','52571':'2025/2026' }[String(v??'')]||String(v??''));
    const seasonStorageValue=v=>({ '2022':'40557','2023':'48982','2024':'58766','2025':'72034','2025/2026':'52571' }[String(v??'')]||String(v??''));
    const marketMap={
    ...
    if(action==='delete_season' && req.method==='POST'){
      ...
      season=seasonStorageValue(season);
...
if(action==='inspect_import' && req.method==='POST'){
    ...
    if(action==='backtest'){
      ...
      if(cfg.filtro_id){
        try{
          ...
          if(cfg.temporada) path+=`&%22TEMPORADA%22=eq.${encodeURIComponent(seasonStorageValue(cfg.temporada))}`;
    ...
    if(temporada&&!/^todos$/i.test(temporada))path+=`&%22TEMPORADA%22=eq.${encodeURIComponent(seasonStorageValue(temporada))}`;
