// Sofa Scrap — GitHub/Render ROOT-ONLY build — V60.5 CACHE-FIRST FILTER ENGINE
...
if(action==='inspect_import' && req.method==='POST'){
    ...
    if(filtroId>0){
      ...
      const runCacheFilter=async (games, filterObj)=>{
        ...
        const ids=[...gamesById.keys()];
        const cachePage=120;
        for(const metricId of metricIds){
          const map=new Map();
          for(let i=0;i<ids.length;i+=cachePage){
            const part=ids.slice(i,i+cachePage);
            const q=`metricas_cache?select=*&metrica_id=eq.${metricId}&jogo_id=in.(${part.join(',')})&limit=${part.length}`;
            let rows=[];
            try{rows=await get(q);}catch(e){return null;}
