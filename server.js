// Sofa Scrap — GitHub/Render ROOT-ONLY build — V60.9 MOTOR HISTÓRICO RÁPIDO
  ...
  try {
    ...
    if(action==='auth_diag'){
      return res.status(200).json({
        ...
        key_mode:secret.startsWith('sb_secret_')?'sb_secret':'jwt_or_anon',
        has_anon:!!anon,
        anon_key_mode:anon?(String(anon).startsWith('sb_secret_')?'sb_secret':'jwt_or_anon'):null,
        service_key_present:!!process.env.SUPABASE_SERVICE_ROLE_KEY,
        secret_key_present:!!process.env.SUPABASE_SECRET_KEY,
        note:'Nenhuma chave é exibida.'
      });
    }
    const headers = secret.startsWith('sb_secret_') ? {apikey: secret} : {apikey: secret, Authorization:`Bearer ${secret}`};
    const anonHeaders = anon ? (String(anon).startsWith('sb_secret_') ? {apikey: anon} : {apikey: anon, Authorization:`Bearer ${anon}`}) : null;
    const get = async (path) => {
      let r=await fetch(`${base}/rest/v1/${path}`,{headers,cache:'no-store'});
      let d={}; try{d=await r.json()}catch{}
      if(!r.ok && /PGRST303|JWT issued at future/i.test(JSON.stringify(d)) && anonHeaders && JSON.stringify(anonHeaders)!==JSON.stringify(headers)){
        r=await fetch(`${base}/rest/v1/${path}`,{headers:anonHeaders,cache:'no-store'});
    ...
    if(action==='engine_test') {
      const filtroId=Number(p.filtro_id||0);
      if(!Number.isInteger(filtroId)||filtroId<=0) return res.status(400).json({error:'Informe filtro_id válido.'});
      const t0=Date.now();
      const [filters,nodes,conds,rels,configs]=await Promise.all([
        get(`filtros?id=eq.${filtroId}&select=id,nome,ativo,campeonatos&limit=1`),
        get(`filtro_nos?filtro_id=eq.${filtroId}&select=*&order=ordem.asc,id.asc&limit=5000`),
        get(`filtro_condicoes?select=*&limit=10000`),
        get(`filtro_nos_relacoes?select=*&limit=10000`),
        get('metricas_config?select=*&limit=1000')
      ]);
      const filtro=filters[0];
      if(!filtro) return res.status(404).json({error:'Filtro não encontrado.'});
      if(filtro.ativo===false) return res.status(400).json({error:'Filtro inativo.'});
      const nodeIds=new Set(nodes.map(x=>Number(x.id)));
      const conditions=conds.filter(x=>nodeIds.has(Number(x.no_id)));
      const rel=rels.filter(x=>nodeIds.has(Number(x.no_pai_id))||nodeIds.has(Number(x.no_filho_id)));
      const cfgById=new Map(configs.map(x=>[Number(x.id),x]));
      const metricCache=new Map();
      const metricIds=new Set();
      const collectMetric=(id,depth=0)=>{
        if(!id||depth>12||metricIds.has(Number(id)))return;
        metricIds.add(Number(id));
        const cfg=cfgById.get(Number(id));
        if(cfg?.formula?.tipo==='COMPOSTA') for(const item of (cfg.formula.componentes||[])) collectMetric(Number(item.metrica_id),depth+1);
      };
      for(const c of conditions){collectMetric(c.metrica_id);collectMetric(c.metrica_comparada_id);}
      const core=['id','DATA','CONFRONTO','CAMPEONATO','TEMPORADA','GOLS_CASA_FT','GOLS_VISITANTE_FT'];
      const fields=new Set(core);
      for(const id of metricIds){
        const cfg=cfgById.get(id); if(!cfg)continue;
        const campo=String(cfg.campo||'').trim();
        if(campo && campo.toUpperCase()!=='COMPOSTA' && campo.toUpperCase()!=='CUSTO_PONTO') fields.add(campo);
        if(campo.endsWith('_CASA')) fields.add(campo.replace(/_CASA$/,'_VISITANTE'));
        if(campo.endsWith('_VISITANTE')) fields.add(campo.replace(/_VISITANTE$/,'_CASA'));
        if(String(cfg.formula?.odd||'').toUpperCase()==='ODD_1X2') fields.add('ODD_1X2_CASA'),fields.add('ODD_1X2_VISITANTE');
      }
      const select=[...fields].map(x=>`"${x.replace(/"/g,'')}"`).join(',');
      const cacheKey=`${select}|DATA_ASC_ID_ASC`;
      const now=Date.now();
      let cached=ENGINE_HISTORY_CACHE.get(cacheKey);
      let jogos;
      let source='cache';
      if(cached && cached.createdAt && now-cached.createdAt < 10*60*1000 && Array.isArray(cached.jogos) && cached.jogos.length){
        jogos=cached.jogos;
      }else{
        source='supabase_recent_window';
        // O teste anterior baixava os ~15 mil jogos antes de aplicar `sample`,
        // causando statement timeout no Supabase. Para o teste do motor novo,
        // basta carregar uma janela recente que contenha os jogos avaliados e
        // o histórico necessário para as janelas configuradas das métricas.
        const requestedSample=Math.max(1,Math.min(Number(p.sample||25),500));
        const historyRows=Math.max(2000,Math.min(Number(p.history||5000),10000),requestedSample+500);
        const basePath=`jogos?select=${select}&order="DATA".desc,"id".desc&limit=${historyRows}`;
        const recent=await get(basePath);
        jogos=(Array.isArray(recent)?recent:[]).slice().reverse();
        ENGINE_HISTORY_CACHE.set(cacheKey,{createdAt:Date.now(),jogos});
      }
      const sampleCount=Math.max(1,Math.min(Number(p.sample||25),Math.min(500,jogos.length)));
      const jogosAvaliados=jogos.slice(-sampleCount);
      const normalize=(v,def)=>{const s=String(v??'').trim().toUpperCase();return ['VISITANTE','FORA','AWAY'].includes(s)?'VISITANTE':['CASA','HOME'].includes(s)?'CASA':def};
      const teamName=(j,side)=>{const parts=String(j.CONFRONTO||'').split(/\s+x\s+/i);return String(parts[side==='CASA'?0:1]||'').trim().toLowerCase();};

      // V60.9 — índice histórico em memória.
      // Antes, histMatches() fazia jogos.filter(...).sort(...) para CADA jogo
      // avaliado. Em 15k jogos isso virava O(N²) e era o principal gargalo.
      // Agora cada combinação equipe/contexto é montada e ordenada uma única vez.
      const indexT0=Date.now();
      let historyIndex,historyPos;
      if(cached && cached.historyIndex && cached.historyPos){
        historyIndex=cached.historyIndex;
        historyPos=cached.historyPos;
      }else{
        historyIndex=new Map();
        historyPos=new Map();
        const addHistory=(key,h)=>{
          if(!key)return;
          let arr=historyIndex.get(key);
          if(!arr){arr=[];historyIndex.set(key,arr);}
          arr.push(h);
        };
        for(const h of jogos){
          const home=teamName(h,'CASA');
          const away=teamName(h,'VISITANTE');
          if(home){
            addHistory(`${home}|CASA`,h);
            addHistory(`${home}|GERAL`,h);
          }
          if(away){
            addHistory(`${away}|FORA`,h);
            addHistory(`${away}|GERAL`,h);
          }
        }
        for(const [key,arr] of historyIndex){
          arr.sort((a,b)=>String(a.DATA||'').localeCompare(String(b.DATA||''))||Number(a.id)-Number(b.id));
          for(let i=0;i<arr.length;i++) historyPos.set(`${key}|${arr[i].id}`,i);
        }
        const cacheObj=ENGINE_HISTORY_CACHE.get(cacheKey);
        if(cacheObj){cacheObj.historyIndex=historyIndex;cacheObj.historyPos=historyPos;}
      }
      const indexBuildMs=Date.now()-indexT0;
      const histMatches=(j,equipe,contexto)=>{
        const team=teamName(j,equipe); if(!team)return [];
        const key=`${team}|${contexto}`;
        const arr=historyIndex.get(key)||[];
        const pos=historyPos.get(`${key}|${j.id}`);
        if(pos==null) return arr.filter(h=>String(h.DATA||'')<String(j.DATA||'') || (String(h.DATA||'')===String(j.DATA||'')&&Number(h.id)<Number(j.id))).slice(-100);
        return arr.slice(0,pos).reverse();
      };
      const valueForField=(h,field,equipe)=>{
        if(h[field]!=null && field!=='GOLS_CASA_FT' && field!=='GOLS_VISITANTE_FT'){
          const base=field.replace(/_(CASA|VISITANTE)$/,'');
          const side=teamName(h,'CASA')===teamName(h,equipe)?'CASA':(teamName(h,'VISITANTE')===teamName(h,equipe)?'VISITANTE':null);
          if(side && (field.endsWith('_CASA')||field.endsWith('_VISITANTE'))){const actual=`${base}_${side}`;return num(h[actual]);}
        }
        if(field==='GOLS_CASA_FT'||field==='GOLS_VISITANTE_FT'){
          const side=teamName(h,'CASA')===teamName(h,equipe)?'CASA':(teamName(h,'VISITANTE')===teamName(h,equipe)?'VISITANTE':null);
          return side==='CASA'?num(h.GOLS_CASA_FT):side==='VISITANTE'?num(h.GOLS_VISITANTE_FT):null;
        }
        return num(h[field]);
      };
      const calc=(cfgId,j,eqOverride=null,ctxOverride=null,depth=0)=>{
        const key=`${cfgId}|${j.id}|${eqOverride||''}|${ctxOverride||''}`; if(metricCache.has(key))return metricCache.get(key);
        if(depth>12){metricCache.set(key,null);return null;}
        const cfg=cfgById.get(Number(cfgId)); if(!cfg||cfg.ativa===false){metricCache.set(key,null);return null;}
        const equipe=normalize(eqOverride||cfg.equipe,'CASA'); const contexto=normalize(ctxOverride||cfg.contexto,'GERAL');
        const tipo=String(cfg.formula?.tipo||'NORMAL').toUpperCase();
        let vals=[];
        if(tipo==='COMPOSTA'){
          for(const item of (cfg.formula?.componentes||[])){const v=calc(Number(item.metrica_id),j,equipe,contexto,depth+1);if(v!=null)vals.push(v);}
        } else if(tipo==='CUSTO_PONTO' || String(cfg.campo||'').toUpperCase()==='CUSTO_PONTO'){
          const hist=histMatches(j,equipe,contexto).slice(0,Number(cfg.janela_jogos||5));
          if(hist.length<Number(cfg.minimo_jogos||1)){metricCache.set(key,null);return null;}
          const oddField=equipe==='CASA'?'ODD_1X2_CASA':'ODD_1X2_VISITANTE';
          vals=hist.map(h=>{const odd=num(h[oddField]);if(odd==null||odd<=1)return null;const home=teamName(h,'CASA')===teamName(h,equipe);const hg=num(h.GOLS_CASA_FT),ag=num(h.GOLS_VISITANTE_FT);if(hg==null||ag==null)return null;const pts=home?(hg>ag?3:hg===ag?1:0):(ag>hg?3:ag===hg?1:0);return (1/odd)*pts;}).filter(v=>v!=null);
        } else {
          const hist=histMatches(j,equipe,contexto).slice(0,Number(cfg.janela_jogos||5));
          if(hist.length<Number(cfg.minimo_jogos||1)){metricCache.set(key,null);return null;}
          vals=hist.map(h=>valueForField(h,String(cfg.campo||''),equipe)).filter(v=>v!=null);
        }
        if(vals.length<Number(cfg.minimo_jogos||1)){metricCache.set(key,null);return null;}
        const op=String(cfg.operacao||cfg.formula?.operacao||'MEDIA').toUpperCase();
        let out=null;
        if(tipo==='COMPOSTA'){
          if(!vals.length)out=null; else if(op==='MEDIA')out=vals.reduce((a,v)=>a+v,0)/vals.length; else if(op==='SOMA')out=vals.reduce((a,v)=>a+v,0); else if(op==='SUBTRACAO'||op==='DIFERENCA')out=vals.slice(1).reduce((a,v)=>a-v,vals[0]); else if(op==='MULTIPLICACAO'||op==='PRODUTO')out=vals.slice(1).reduce((a,v)=>a*v,vals[0]); else if(op==='DIVISAO')out=vals.slice(1).reduce((a,v)=>v===0?null:a/v,vals[0]);
        } else if(op==='MEDIA')out=vals.reduce((a,v)=>a+v,0)/vals.length;
        else if(op==='SOMA')out=vals.reduce((a,v)=>a+v,0);
        else if(op==='CV'){const mean=vals.reduce((a,v)=>a+v,0)/vals.length;const variance=vals.reduce((a,v)=>a+(v-mean)**2,0)/vals.length;out=mean===0?null:Math.sqrt(variance)/Math.abs(mean)*100;}
        else if(op==='MIN')out=Math.min(...vals); else if(op==='MAX')out=Math.max(...vals);
        metricCache.set(key,out);return out;
      };
      const nodeById=new Map(nodes.map(n=>[Number(n.id),n]));
      const children=new Map(); for(const r of rel){const k=Number(r.no_pai_id);if(!children.has(k))children.set(k,[]);children.get(k).push({id:Number(r.no_filho_id),ordem:Number(r.ordem||0)});} for(const a of children.values())a.sort((x,y)=>x.ordem-y.ordem||x.id-y.id);
      const condByNode=new Map(conditions.map(c=>[Number(c.no_id),c]));
      const evalNode=(id,j)=>{const n=nodeById.get(Number(id));if(!n)return false;if(String(n.tipo||'').toUpperCase()==='CONDICAO'){const c=condByNode.get(Number(id));if(!c)return false;const eq=normalize(c.equipe,'CASA'),ctx=normalize(c.contexto,'GERAL');const a=calc(Number(c.metrica_id),j,eq,ctx);if(a==null)return false;let target=num(c.valor);if(c.metrica_comparada_id!=null){const b=calc(Number(c.metrica_comparada_id),j,eq,ctx);if(b==null)return false;target=b*(1+Number(c.percentual||0)/100);}const op=String(c.operador||'>');if(op==='>')return a>target;if(op==='>=')return a>=target;if(op==='<')return a<target;if(op==='<=')return a<=target;if(op==='=')return a===target;if(op==='<>')return a!==target;return false;}const kids=children.get(Number(id))||[];if(!kids.length)return false;const vals=kids.map(x=>evalNode(x.id,j));return String(n.operador_logico||'E').toUpperCase()==='OU'?vals.some(Boolean):vals.every(Boolean);};
      const childIds=new Set([...children.values()].flat().map(x=>x.id));
      const roots=nodes.filter(n=>!childIds.has(Number(n.id))).sort((a,b)=>Number(a.ordem||0)-Number(b.ordem||0)||Number(a.id)-Number(b.id));
      const approved=[]; for(const j of jogosAvaliados){if(roots.length&&roots.every(r=>evalNode(r.id,j)))approved.push(j.id);}
      let probe=null;
      if(String(p.probe||'')==='1' && jogosAvaliados.length){
        const j=jogosAvaliados[0];
        const c=conditions[0]||null;
        if(c){
          const eq=normalize(c.equipe,'CASA');
          const ctx=normalize(c.contexto,'GERAL');
          const cfg=cfgById.get(Number(c.metrica_id));
          const hist=histMatches(j,eq,ctx);
          const used=hist.slice(0,Number(cfg?.janela_jogos||5));
          const valor=calc(Number(c.metrica_id),j,eq,ctx);
          let alvo=num(c.valor);
          let comparada=null;
          if(c.metrica_comparada_id!=null){
            comparada=calc(Number(c.metrica_comparada_id),j,eq,ctx);
            if(comparada!=null) alvo=comparada*(1+Number(c.percentual||0)/100);
          }
          probe={
            jogo:{id:j.id,data:j.DATA,confronto:j.CONFRONTO,campeonato:j.CAMPEONATO},
            condicao:{no_id:c.no_id,metrica_id:c.metrica_id,operador:c.operador,valor_configurado:c.valor,equipe:eq,contexto:ctx,janela:cfg?.janela_jogos,minimo:cfg?.minimo_jogos,operacao:cfg?.operacao},
            valor_calculado:valor,
            alvo:alvo,
            passa:valor!=null ? (c.operador==='>=' ? valor>=alvo : c.operador==='>' ? valor>alvo : c.operador==='<' ? valor<alvo : c.operador==='<=' ? valor<=alvo : c.operador==='=' ? valor===alvo : valor!==alvo) : false,
            historico_considerado:used.map(h=>({id:h.id,data:h.DATA,confronto:h.CONFRONTO})),
            historico_disponivel:hist.length
          };
        }
      }
      let comparison=null;
      if(String(p.compare||'')==='1'){
        // Comparação limitada para diagnóstico: chamar o motor SQL antigo para
        // todos os jogos recriaria o gargalo que estamos justamente evitando.
        const compareIds=jogosAvaliados.slice(0,Math.min(jogosAvaliados.length,20)).map(j=>Number(j.id));
        const oldResults=[];
        for(const id of compareIds){
          try{
            const r=await post('rpc/avaliar_filtro',{p_filtro_id:filtroId,p_jogo_id:id},'POST');
            const ok=r===true||r===1||r==='true'||(Array.isArray(r)&&((r[0]===true)||(r[0]===1)||(r[0]?.avaliar_filtro===true)||(r[0]?.resultado===true)))||r?.avaliar_filtro===true||r?.avaliar_filtro===1||r?.avaliar_filtro==='true'||r?.resultado===true||r?.resultado===1||r?.resultado==='true'||r?.ok===true;
            oldResults.push({id,old:ok,new:approved.includes(id)});
          }catch(e){oldResults.push({id,old:null,new:approved.includes(id),error:e.message||String(e)});}
        }
        const valid=oldResults.filter(x=>x.old!==null);
        comparison={requested:compareIds.length,valid:valid.length,old_aprovados:valid.filter(x=>x.old).length,new_aprovados:valid.filter(x=>x.new).length,diferentes:valid.filter(x=>x.old!==x.new).length,iguais:valid.length>0&&valid.every(x=>x.old===x.new),amostra:oldResults};
      }
      return res.status(200).json({ok:true,engine:'NODE_V60_12_SHADOW',filtro_id:filtroId,filtro_nome:filtro.nome,jogos_total:jogos.length,jogos_avaliados:jogosAvaliados.length,jogos_aprovados:approved.length,ids:approved.slice(0,20),metricas_usadas:[...metricIds],campos_carregados:[...fields],index_chaves:historyIndex.size,index_build_ms:indexBuildMs,tempo_ms:Date.now()-t0,cache_origem:source,cache_ttl_min:10,history_window:jogos.length,probe,comparison,observacao:'Motor novo em modo sombra com janela histórica limitada para teste e cache em memória. Nenhuma função SQL nem resultado do dashboard foi alterado.'});
    }
    if(action==='db_count') {
...
;
    ...
    if(action==='save_metric'){
      ...
      const created=await post('metricas_config',{
        nome:body.nome,descricao:body.descricao||'',campo:tipo==='COMPOSTA'?'COMPOSTA':campo,equipe:body.equipe||'CASA',contexto:body.contexto||'GERAL',
    ...
    if(action==='update_metric'){
      ...
      const updated=await post(`metricas_config?id=eq.${id}`,{
        nome,descricao:body.descricao||'',campo:tipo==='COMPOSTA'?'COMPOSTA':campo,equipe:body.equipe||'CASA',contexto:body.contexto||'GERAL',
        janela_jogos:Number(body.janela_jogos||5),minimo_jogos:Number(body.minimo_jogos||3),
    ...
    if(action==='save_filter'){
      ...
      for(let i=0;i<conds.length;i++){
        ...
        await post('filtro_condicoes',{
          ...
          equipe:c.equipe||'CASA',contexto:c.contexto||'GERAL',janela:Number(c.janela||5),minimo:Number(c.minimo||3),
...
if(action==='inspect_import' && req.method==='POST'){
    ...
    if(action==='import_games' && req.method==='POST'){
      ...
      if(unique.length){
        ...
        for(let i=0;i<unique.length;i+=50){
          ...
          await post('jogos',part,'POST');
          inserted+=part.length;
        }
      }
      return res.status(200).json({ok:true,received:incoming.length,inserted,skipped,errors});
    }
    ...
    if(filtroId>0){
      ...
      try{
        // Usa o mesmo RPC em lote que já era usado pela V45. Ele avalia o filtro
        // inteiro dentro do PostgreSQL e devolve os IDs aprovados, sem limitar o
        // dashboard aos primeiros 5.000 jogos.
        const result=await post('rpc/executar_filtro_jogos',{p_filtro_id:filtroId},'POST');
        // O Banco B teve versões diferentes da RPC: algumas retornam um array
        // diretamente, outras devolvem {jogo_id}, {id}, {ids:[...]},
        // {results:[...]}, {data:[...]} ou até JSON serializado.
        // Se não conseguirmos extrair IDs, NÃO transformamos um filtro válido
        // em zero jogos: caímos para a avaliação oficial jogo a jogo.
        const normalizeFilterIds=(value)=>{
          let v=value;
          if(typeof v==='string'){try{v=JSON.parse(v)}catch{}}
          if(Array.isArray(v)) return v;
          if(v==null) return [];
          if(v.jogo_id!=null||v.id!=null) return [v];
          for(const k of ['ids','jogo_ids','results','result','data','rows','jogos']){
            if(v?.[k]!=null){const x=normalizeFilterIds(v[k]);if(x.length)return x;}
          }
          return [];
        };
        const list=normalizeFilterIds(result);
        const ids=list.map(x=>{
          if(typeof x==='number'||typeof x==='string') return String(x);
          return String(x?.jogo_id??x?.id??x?.jogo?.id??'');
        }).filter(Boolean);
        if(ids.length) approvedIds=new Set(ids);
      }catch(e){
        console.warn(`RPC em lote do filtro #${filtroId} não pôde ser usada; usando avaliação individual: ${e.message||e}`);
      }
      // Se a RPC em lote não devolveu IDs em formato reconhecido, avaliamos
      // cada jogo pela função oficial. Isso evita o P/L zerado por incompatibilidade
      // de retorno entre versões do PostgREST/RPC.
      if(!approvedIds){
        const approved=[]; const chunkSize=25;
        for(let i=0;i<data.length;i+=chunkSize){
          const chunk=data.slice(i,i+chunkSize);
          const results=await Promise.all(chunk.map(async j=>{
            try{
              const r=await post('rpc/avaliar_filtro',{p_filtro_id:filtroId,p_jogo_id:Number(j.id)},'POST');
              const ok=r===true||r===1||r==='true'||
                (Array.isArray(r)&&((r[0]===true)||(r[0]===1)||(r[0]?.avaliar_filtro===true)||(r[0]?.resultado===true)))||
                r?.avaliar_filtro===true||r?.avaliar_filtro===1||r?.avaliar_filtro==='true'||
                r?.resultado===true||r?.resultado===1||r?.resultado==='true'||r?.ok===true;
              return ok?j:null;
            }catch(e){
              throw new Error(`Falha ao avaliar o filtro no jogo ${j.id}: ${e.message||e}`);
            }
          }));
          results.forEach(j=>{if(j)approved.push(j)});
        }
        data=approved;
      }else{
        data=data.filter(j=>approvedIds.has(String(j.id)));
      }
    }
