...
function fairPnl(j,market,stake=1,commission=.045){const f=fairOdd(j,market);const mc=marketMap[market];if(f===null||!mc)return null;const g=num(j.GOLS_CASA_FT),h=num(j.GOLS_VISITANTE_FT);if(g===null||h===null)return null;const green=mc.win(g,h);const gross=(green?f-1:-1)*stake;const net=gross>0?gross*(1-commission):gross;return {fairOdd:f,fairPnlGross:gross,fairPnlNet:net};}
  try {
    ...
    if(action==='engine_test') {
      ...
      const approved=[]; for(const j of jogos){if(roots.length&&roots.every(r=>evalNode(r.id,j)))approved.push(j.id);}
      return res.status(200).json({ok:true,engine:'NODE_V60_SHADOW',filtro_id:filtroId,filtro_nome:filtro.nome,jogos_total:jogos.length,jogos_aprovados:approved.length,ids:approved.slice(0,20),metricas_usadas:[...metricIds],campos_carregados:[...fields],tempo_ms:Date.now()-t0,observacao:'Motor novo em modo sombra; nenhuma função SQL nem resultado do dashboard foi alterado.'});
    }
    if(action==='db_count') {
