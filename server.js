// Sofa Scrap — GitHub/Render ROOT-ONLY build — PERFORMANCE + P/L FILTER FIX 02
...
function fairPnl(j,market,stake=1,commission=.045){const f=fairOdd(j,market);const mc=marketMap[market];if(f===null||!mc)return null;const g=num(j.GOLS_CASA_FT),h=num(j.GOLS_VISITANTE_FT);if(g===null||h===null)return null;const green=mc.win(g,h);const gross=(green?f-1:-1)*stake;const net=gross>0?gross*(1-commission):gross;return {fairOdd:f,fairPnlGross:gross,fairPnlNet:net};}
  try {
    ...
    const getAll = async (path, pageSize=1000, maxRows=50000) => {
      ...
      // O dashboard tinha 19k+ jogos e buscava as páginas do Supabase uma por uma.
      // Isso fazia cada carregamento esperar dezenas de requisições sequenciais.
      // Buscamos pequenos lotes de páginas em paralelo e preservamos a ordem.
      const parallelPages=8;
      for(let batchStart=0; out.length<maxRows; batchStart+=parallelPages*pageSize){
        const offsets=[];
        for(let i=0;i<parallelPages && batchStart+i*pageSize<maxRows;i++) offsets.push(batchStart+i*pageSize);
        const pages=await Promise.all(offsets.map(offset=>get(`${clean}${sep}limit=${pageSize}&offset=${offset}`)));
        let reachedEnd=false;
        for(let i=0;i<pages.length;i++){
          const page=pages[i];
          if(!Array.isArray(page)||!page.length){reachedEnd=true;break;}
          out.push(...page);
...
if(action==='inspect_import' && req.method==='POST'){
    ...
    if(filtroId>0){
      ...
      const candidateIds=data.map(j=>Number(j.id)).filter(Number.isFinite);

      // Caminho principal: mesma RPC que funcionava na base V46.
      // Ela usa o motor oficial do Banco B dentro do PostgreSQL e evita
      // milhares de chamadas HTTP individuais.
      ...
      const extractFilterIds=(value)=>normalizeFilterIds(value)
        .map(x=>Number(typeof x==='number'||typeof x==='string'?x:x?.jogo_id??x?.id??x?.jogo?.id))
        .filter(Number.isFinite);

      try{
        const result=await post('rpc/executar_filtro_jogos',{p_filtro_id:filtroId},'POST');
        const ids=extractFilterIds(result);
        if(ids.length){
          approvedIds=new Set(ids.map(String));
        }
      }catch(primaryError){
        // A RPC V46 pode não existir em alguma instalação. Nesse caso,
        // usamos a RPC V60 em lotes menores, sem cair em avaliação HTTP jogo a jogo.
        try{
          const approved=[];
          const batchSize=500;
          for(let i=0;i<candidateIds.length;i+=batchSize){
            const idsChunk=candidateIds.slice(i,i+batchSize);
            const result=await post('rpc/avaliar_filtro_lote',{p_filtro_id:filtroId,p_jogo_ids:idsChunk},'POST');
            approved.push(...extractFilterIds(result));
          }
          if(approved.length) approvedIds=new Set(approved.map(String));
        }catch(batchError){
          throw new Error(`Falha ao aplicar o filtro #${filtroId}: ${primaryError.message||String(primaryError)} | lote: ${batchError.message||String(batchError)}`);
        }
      }

      // Se a RPC respondeu vazio sem erro, não assumimos imediatamente que
      // o filtro não encontrou jogos: tentamos a RPC em lote como segunda via.
      // Isso corrige o caso em que uma versão do PostgREST devolve [] mesmo
      // havendo jogos aprovados.
      if(!approvedIds && candidateIds.length){
        try{
          const approved=[];
          const batchSize=500;
          for(let i=0;i<candidateIds.length;i+=batchSize){
          ...
          // Mantém approvedIds nulo para produzir 0 somente se as duas vias
          // realmente não conseguirem localizar jogos aprovados.
        }
      }

      if(!approvedIds) approvedIds=new Set();
