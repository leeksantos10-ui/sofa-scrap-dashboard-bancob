...
if(action==='inspect_import' && req.method==='POST'){
    ...
    if(filtroId>0){
      ...
      try{
        // Banco B possui a RPC em lote avaliar_filtro_lote(p_filtro_id bigint, p_jogo_ids bigint[])
        // que executa avaliar_filtro dentro do PostgreSQL. Isso substitui centenas de
        // requisições HTTP individuais e evita que o dashboard fique travado.
        ...
        for(let i=0;i<candidateIds.length;i+=batchSize){
          ...
          const list=Array.isArray(result)?result:(result?.jogo_id!=null?[result]:[]);
          list.forEach(x=>{
            const id=Number(typeof x==='number'||typeof x==='string'?x:x?.jogo_id??x?.id);
            if(Number.isFinite(id)) approved.push(id);
          });
        }
        approvedIds=new Set(approved.map(String));
      }catch(e){
        // Compatibilidade: se a RPC em lote não estiver disponível, tentamos a RPC
        // antiga em lote. Nunca fazemos avaliação jogo a jogo de milhares de registros.
        try{
          ...
          const ids=list.map(x=>String(typeof x==='number'||typeof x==='string'?x:x?.jogo_id??x?.id??x?.jogo?.id??'')).filter(Boolean);
          approvedIds=new Set(ids);
        }catch(e2){
          throw new Error(`Falha ao aplicar o filtro #${filtroId}: ${e.message||e2.message||String(e2)}`);
        }
      }
