// Sofa Scrap — GitHub/Render ROOT-ONLY build — V60.7 CACHE-FIRST FILTER ENGINE
    ...
    if(filtroId>0){
      ...
      const runCacheFilter=async (games, filterObj)=>{
        ...
        for(const metricId of metricIds){
          ...
          let rows=[];
          try{
            rows=await getAll(`metricas_cache?select=*&metrica_id=eq.${metricId}`,1000,50000);
          }catch(e){
          ...
          for(const row of (rows||[])){
            ...
            map.set(`${row.jogo_id}|${normalizeText(row.equipe)}|${normalizeText(row.contexto)}|${Number(row.janela??0)}|${Number(row.minimo??0)}|${normalizeText(row.operacao)}`,val);
          }
        ...
        const getCached=(j,cond)=>{
          ...
          const key=`${j.id}|${normalizeText(cond.equipe||'CASA')}|${normalizeText(cond.contexto||'GERAL')}|${Number(cond.janela||5)}|${Number(cond.minimo||3)}|${normalizeText(cond.operacao||'MEDIA')}`;
          return map.has(key)?map.get(key):null;
        };
        // Se houver lacunas de cache, preenchemos somente essas combinações pelo
        // motor V59. Isso evita recalcular histórico quando o cache já existe.
        const missing=[];
        ...
        for(let i=0;i<missing.length;i+=concurrency){
          ...
          await Promise.all(batch.map(async ({j,c})=>{
            ...
            if(val!==null){
              valuesByMetric.get(Number(c.metrica_id))?.set(`${j.id}|${normalizeText(eq)}|${normalizeText(ctx)}|${Number(c.janela||mc.janela_jogos||5)}|${Number(c.minimo||mc.minimo_jogos||3)}|${normalizeText(c.operacao||mc.operacao||'MEDIA')}`,val);
            }
          }));
        }
