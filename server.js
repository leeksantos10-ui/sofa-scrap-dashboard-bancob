// Sofa Scrap — GitHub/Render ROOT-ONLY build
...
if(action==='inspect_import' && req.method==='POST'){
    ...
    if(filtroId>0){
      ...
      try{
        ...
        approvedIds=new Set(list.map(x=>String(x?.jogo_id??x?.id??x)).filter(Boolean));
      }catch(e){
        const msg=String(e.message||e);
        if(/executar_filtro_jogos|404|not found|does not exist|PGRST/i.test(msg)){
          // Compatibilidade com Banco B que ainda não tenha a RPC em lote.
          ...
          data=approved;
        }else throw new Error(`Falha ao executar o filtro #${filtroId}: ${msg}`);
      }
      if(approvedIds) data=data.filter(j=>approvedIds.has(String(j.id)));
    }
