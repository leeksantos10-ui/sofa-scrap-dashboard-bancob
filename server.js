...
if(action==='inspect_import' && req.method==='POST'){
    ...
    if(filtroId>0){
      ...
      // Primeiro usa a RPC consolidada do Banco B (a mesma rota do V46),
      // que avalia o filtro inteiro dentro do PostgreSQL em uma única chamada.
      // Isso é mais confiável para filtros já existentes e evita milhares de
      // requisições HTTP. A RPC em lote nova fica apenas como compatibilidade.
      ...
      try{
        ...
        // Compatibilidade com Bancos que já receberam a RPC nova em lote.
        const candidateIds=data.map(j=>Number(j.id)).filter(Number.isFinite);
        try{
          const approved=[];
          const batchSize=5000;
          for(let i=0;i<candidateIds.length;i+=batchSize){
            const result=await post('rpc/avaliar_filtro_lote',{p_filtro_id:filtroId,p_jogo_ids:candidateIds.slice(i,i+batchSize)},'POST');
            for(const x of normalizeFilterIds(result)){
              const id=Number(typeof x==='number'||typeof x==='string' ? x : (x?.jogo_id??x?.id??''));
              if(Number.isFinite(id)) approved.push(String(id));
            }
          }
