...
function fairPnl(j,market,stake=1,commission=.045){const f=fairOdd(j,market);const mc=marketMap[market];if(f===null||!mc)return null;const g=num(j.GOLS_CASA_FT),h=num(j.GOLS_VISITANTE_FT);if(g===null||h===null)return null;const green=mc.win(g,h);const gross=(green?f-1:-1)*stake;const net=gross>0?gross*(1-commission):gross;return {fairOdd:f,fairPnlGross:gross,fairPnlNet:net};}
  try {
    ...
    if(action==='engine_test') {
      ...
      let jogos;
      let source='cache';
      if(cached && now-cached.createdAt < 10*60*1000){
        jogos=cached.jogos;
      }else{
        source='supabase';
        const basePath=`jogos?select=${select}&order="DATA".asc,"id".asc`;
        const pageSize=1000, maxRows=50000;
        const fetchPage=async(offset)=>get(`${basePath}&limit=${pageSize}&offset=${offset}`);
        const out=[];
        // Busca em pequenos lotes paralelos para não ficar esperando 16 requisições em série.
        for(let start=0; out.length<maxRows; start+=6*pageSize){
          const offsets=[];
          for(let k=0;k<6 && out.length<maxRows;k++) offsets.push(start+k*pageSize);
          const pages=await Promise.all(offsets.map(fetchPage));
          let ended=false;
          for(const page of pages){
            if(!Array.isArray(page)||!page.length){ended=true;break;}
            out.push(...page);
            if(page.length<pageSize){ended=true;break;}
            if(out.length>=maxRows){ended=true;break;}
          }
          if(ended) break;
        }
        jogos=out.slice(0,maxRows);
        ENGINE_HISTORY_CACHE.set(cacheKey,{createdAt:Date.now(),jogos});
      }
      const sampleCount=Math.max(1,Math.min(Number(p.sample||25),jogos.length));
      ...
      if(String(p.compare||'')==='1'){
        const oldRaw=await post('rpc/executar_filtro_jogos',{p_filtro_id:filtroId},'POST');
        const oldArr=Array.isArray(oldRaw)?oldRaw:(oldRaw?.data||[]);
        const normalizeId=x=>Number(x?.jogo_id ?? x?.id ?? x);
        const oldIds=oldArr.map(normalizeId).filter(Number.isFinite);
        const newSet=new Set(approved.map(Number));
        const oldSet=new Set(oldIds);
        const onlyNew=approved.filter(id=>!oldSet.has(Number(id))).map(Number);
        const onlyOld=oldIds.filter(id=>!newSet.has(Number(id)));
        let same=0; for(const id of newSet) if(oldSet.has(id)) same++;
        compare={old_count:oldSet.size,new_count:newSet.size,intersection:same,only_new_count:onlyNew.length,only_old_count:onlyOld.length,only_new_sample:onlyNew.slice(0,20),only_old_sample:onlyOld.slice(0,20),iguais:newSet.size===oldSet.size&&same===newSet.size};
      }
      return res.status(200).json({ok:true,engine:'NODE_V60_12_SHADOW',filtro_id:filtroId,filtro_nome:filtro.nome,jogos_total:jogos.length,jogos_avaliados:jogosAvaliados.length,jogos_aprovados:approved.length,ids:approved.slice(0,20),metricas_usadas:[...metricIds],campos_carregados:[...fields],index_chaves:historyIndex.size,index_build_ms:indexBuildMs,tempo_ms:Date.now()-t0,cache_origem:source,cache_ttl_min:10,probe,compare,observacao:'Motor novo em modo sombra. V60.12 adiciona somente comparação diagnóstica com executar_filtro_jogos quando compare=1. Nenhuma função SQL nem resultado do dashboard foi alterado.'});
    }
    if(action==='db_count') {
